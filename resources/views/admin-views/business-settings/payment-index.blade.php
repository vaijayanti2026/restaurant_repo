...
@section('content')
    <div class="content container-fluid">
        ...
        <div class="row g-2">
            ...
            <div class="col-md-6">
                <div class="card">
                    @php($config=\App\CentralLogics\Helpers::get_business_settings('square') ?? [])
                    <form
                        action="{{env('APP_MODE')!='demo'?route('admin.business-settings.web-app.payment-method-update',['square']):'javascript:'}}"
                        method="post">
                        @csrf
                        <div class="card-body">
                            <div class="d-flex justify-content-between">
                                <h5 class="text-uppercase mb-4">{{translate('square')}}</h5>

                                <label class="switcher">
                                    <input class="switcher_input" name="status" type="checkbox" {{($config['status'] ?? 1) == 1? 'checked' : ''}}>
                                    <span class="switcher_control"></span>
                                </label>
                            </div>
                            <center class="mb-4">
                                <img width="140" class="avatar-img" src="{{asset('/public/assets/admin/img/square.png')}}" alt="">
                            </center>
                            @if(isset($config))
                                <div class="form-group">
                                    <label>{{translate('application')}} {{translate('id')}}</label>
                                    <input type="text" class="form-control" name="application_id"
                                           value="{{env('APP_MODE')!='demo'?($config['application_id'] ?? config('services.square.application_id')):''}}">
                                </div>

                                <div class="form-group">
                                    <label>{{translate('access token')}}</label>
                                    <input type="text" class="form-control" name="access_token"
                                           value="{{env('APP_MODE')!='demo'?($config['access_token'] ?? config('services.square.access_token')):''}}">
                                </div>

                                <div class="form-group">
                                    <label>{{translate('location')}} {{translate('id')}}</label>
                                    <input type="text" class="form-control" name="location_id"
                                           value="{{env('APP_MODE')!='demo'?($config['location_id'] ?? config('services.square.location_id')):''}}">
                                </div>

                                <div class="form-group">
                                    <label>{{translate('environment')}}</label>
                                    <select class="form-control" name="environment">
                                        @php($squareEnvironment=$config['environment'] ?? config('services.square.environment', 'sandbox'))
                                ...
                                <div class="form-group">
                                    <label>{{translate('webhook')}} {{translate('signature')}} {{translate('key')}}</label>
                                    <input type="text" class="form-control" name="webhook_signature_key"
                                           value="{{env('APP_MODE')!='demo'?($config['webhook_signature_key'] ?? config('services.square.webhook_signature_key')):''}}">
                                </div>

                                <div class="form-group">
                                    <label>{{translate('webhook')}} URL</label>
                                    <input type="text" class="form-control" name="webhook_url"
                                           value="{{env('APP_MODE')!='demo'?($config['webhook_url'] ?? config('services.square.webhook_url')):''}}">
                                </div>
