        ...
        <div class="d-flex flex-wrap gap-2 align-items-center mb-4">
            <h2 class="h1 mb-0 d-flex align-items-center gap-2">
                ...
                <span class="page-header-title">
                    {{translate('Update_Advertisement')}}
                </span>
            </h2>
        </div>
        <!-- End Page Header -->
                            ...
                            <div class="row">
                                <div class="col-lg-6">
                                     <div class="form-group">
                                        ...
                                        <input type="number" name="trigger_time" class="form-control" 
                                        value="{{$advertisement['trigger_time']}}" placeholder="{{translate('Enter time in seconds')}}" required>
                                    </div>
                                    <div class="form-group">
                                            ...
                                            <select name="status" class="custom-select" required>
                                                ...
                                                <option value="1" {{$advertisement['status']==1? 'selected' : ''}}>{{translate('Active')}}</option>
                                                <option value="0" {{$advertisement['status']==0? 'selected' : ''}}>{{translate('Inactive')}}</option>
                                            </select>
                                        </div>
                                    ...
                                    <div class="form-group">
                                        ...
                                        <select name="item_type" class="custom-select" onchange="show_item(this.value)">
                                            <option value="product" {{$advertisement['product_id']==null?'':'selected'}}>{{translate('product')}}</option>
                                            <option value="category" {{$advertisement['category_id']==null?'':'selected'}}>{{translate('category')}}</option>
                                        </select>
                                    </div>
                                    <div class="form-group" id="type-product"
                                        style="display: {{$advertisement['product_id']==null?'none':'block'}}">
                                        <label class="input-label">{{translate('product')}} <span
                                                class="input-label-secondary">*</span></label>
                                        <select name="product_id" class="custom-select">
                                            @foreach($products as $product)
                                                <option
                                                    value="{{$product['id']}}" {{$advertisement['product_id']==$product['id']?'selected':''}}>
                                                    {{$product['name']}}
                                                </option>
                                            @endforeach
                                        </select>
                                    ...
                                    <div class="form-group" id="type-category"
                                        style="display: {{$advertisement['category_id']==null?'none':'block'}}">
                                        ...
                                        <div class="d-flex justify-content-center mt-4">
                                            <div class="upload-file">
                                                ...
                                                <div class="upload-file__img_drag upload-file__img">
                                                    <img width="465" src="{{asset('storage/app/public/advertisement')}}/{{$advertisement['image']}}" onerror="this.src='{{asset('assets/admin/img/icons/upload_img2.png')}}'" alt="">
                                                </div>
                                            </div>
                                        </div>
                                    </div>
