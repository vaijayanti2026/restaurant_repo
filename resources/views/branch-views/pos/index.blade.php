...
<script>
   ...
   $(function(){
    ...
    $('#order_place').submit(function(eventObj) {
        var form = $(this);
        if($('#customer').val() && form.find('input[name="user_id"]').length === 0)
        {
            form.append('<input type="hidden" name="user_id" value="'+$('#customer').val()+'" /> ');
        }

        var selectedType = form.find('input[name="type"]:checked').val();
        var hasSquareReference = form.find('input[name="transaction_reference"]').val();
        if(selectedType === 'card' && !hasSquareReference)
        {
            eventObj.preventDefault();
            startPosSquarePayment();
            return false;
        }
    ...
    function startPosSquarePayment() {
        $.ajax({
            url: '{{route('branch.pos.square.payment-link')}}',
            type: 'POST',
            data: {
                _token: '{{csrf_token()}}'
            },
            ...
            success: function (data) {
                var paymentLink = data.payment_link || {};
                var paymentUrl = paymentLink.url || paymentLink.long_url;
                if(paymentUrl) {
                ...
                $('#loading').hide();
                toastr.error('Square payment link was not returned.');
            },
            error: function (xhr) {
                $('#loading').hide();
                var message = 'Unable to create Square payment link.';
    ...
    @if(session('pos_square_transaction_reference'))
    setTimeout(function () {
        $('#order_place').submit();
    }, 300);
    @endif
