        ...
        <section class="col-lg-12">
            <div class="mt-3">
                <div class="row">
                    ...
                    @if(false && isset($config) && $config['status'])
                        <div class="col-md-6 mb-4" style="cursor: pointer">
                            <div class="card">
                                <div class="card-body" style="height: 70px">
                                    @php($config=\App\CentralLogics\Helpers::get_business_settings('stripe'))
                    ...
                    @endif
                    @php($squareService=app(\App\Services\SquareService::class))
                    @php($branchId=session('branch_id'))
                    @php($config=$squareService->settings($branchId))
                    @if($squareService->isEnabled($branchId))
                        <div class="col-md-6 mb-4" style="cursor: pointer">
                            <div class="card">
                                <div class="card-body" style="height: 70px">
                                    <button class="btn btn-block click-if-alone" type="button" id="checkout-square">
                                        <img width="100" src="{{asset('public/assets/admin/img/square.png')}}"/>
                                    </button>
                                    <script type="text/javascript">
                                        // Create an instance of the Stripe object with your publishable API key

                                       //// var checkoutButton = document.getElementById("checkout-button");
                                        $("#checkout-square").click(function(){
                                            $.get("{{ route('pay-square',['order_amount'=>$order_amount,'customer_id'=>$customer['id'],'callback'=>$callback,'branch_id'=>session('branch_id')]) }}")
                                                .done(function(data){
                                                    if (data && data.payment_link && (data.payment_link.url || data.payment_link.long_url)) {
                                                        $(location).prop('href', data.payment_link.url || data.payment_link.long_url);
                                                        return;
                                                    }
                                                    toastr.error('Unable to open Square payment.');
                                                })
                                                .fail(function(error){
                                                    var message = error.responseJSON && error.responseJSON.error ? error.responseJSON.error : 'Unable to open Square payment.';
                    ...
                    @if(isset($config) && $config['status'])
                            ...
                            <div class="card">
                                <div class="card-body" style="height: 70px">
                                    <button class="btn btn-block" type="button" data-toggle="modal"
                                        ...
                                        <img width="100" src="{{asset('public/assets/admin/img/wallet.png')}}"/>
                                    </button>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="modal fade" id="exampleModal" tabindex="-1" role="dialog"
                            ...
                            <div class="modal-dialog" role="document">
                                <div class="modal-content" style="background-color:#ee6604;color:#fff;">
                                    <div class="modal-header" style="border-bottom:0px;padding:4px;">
                                        
                                        <button type="button" class="close" data-dismiss="modal" aria-label="Close" style="opacity: 100;">
                                            <!--<span aria-hidden="true">&times;</span>-->
                                            <img src="{{asset('public/assets/icons/close.png')}}" style="width:38px"/>
                                        </button>
                                    </div>
                                    <div class="modal-header" style="padding:0px;border-bottom-color:#ee6604;">
                                        <h3 class="modal-title text-center" id="exampleModalLabel" style="width: 100%;font-weight: 900;">
                                            <p>
                                                <img src="{{asset('public/assets/icons/reward.png')}}" style="width: 94px;"/>
                                            </p>
                                            <p>
                                                {{ translate('Payment by Wallet Point') }}    
                                            </p>
                                            </h3>
                                    </div>
                                    <div class="modal-body">
                                        ...
                                        <hr style="border-top: 2px solid #ee6604;background:#ee6604;">
                                        <center>
                                    ...
                                    <div class="modal-footer text-center" style="justify-content: center;">
                                        <button type="button" style="background-color:#828282;font-weight:500;color:#fff; width:100px;" class="btn " data-dismiss="modal">{{ translate('Close') }}
                                        </button>
                                        @if(($value*$order_amount)<=$point)
                                            <form action="{!! route('internal-point-pay',['order_amount'=>$order_amount,'customer_id'=>$customer['id'],'callback'=>$callback]) !!}" method="POST">
                                                ...
                                                <button type="submit" style="background:#fffcf0;font-weight:500;color:#000;width:100px;" class="btn  click-if-alone">{{ translate('Proceed') }}</button>
                                            </form>
                                        @else
