    ...
    public function mail_config(Request $request)
    {
        ...
        BusinessSetting::where(['key' => 'mail_config'])->update([
            'value' => json_encode([
                ...
                'mail_cc_sec'=>$request['mail_cc_sec']
            ]),
        ]);
...
//        return $request;
        ...
        elseif ($name == 'square') {
            $payment = BusinessSetting::where('key', 'square')->first();
            if (isset($payment) == false) {
                DB::table('business_settings')->insert([
                    'key' => 'square',
                    'value' => json_encode([
                        ...
                        'webhook_signature_key' => $request['webhook_signature_key'],
                ...
                DB::table('business_settings')->where(['key' => 'square'])->update([
                    'key' => 'square',
                    'value' => json_encode([
                        'status' => $request['status'] == 'on' ? 1 : 0,
                        'application_id' => $request['application_id'],
...
//     // 🔥 NEW: Create settings array with import_pos_orders
//     $squareSettings = [
//         'status' => $request['status'] == 'on' ? 1 : 0,
//         'application_id' => $request['application_id'],
//         'access_token' => $request['access_token'],
...
//         'webhook_signature_key' => $request['webhook_signature_key'],
//         'webhook_url' => $request['webhook_url'],
//         'import_pos_orders' => $request->has('import_pos_orders') ? 1 : 0, // 🔥 NEW LINE
//     ];
    
//     if (isset($payment) == false) {
//         DB::table('business_settings')->insert([
//             'key' => 'square',
//             'value' => json_encode($squareSettings),
//             'created_at' => now(),
//             'updated_at' => now(),
//         ]);
//     } else {
...
//        return $request;
    ...
    public function branchSettings(){
        return view('admin-views.business-settings.branch_settings');
    }
    public function generalSettings(){
        return view('admin-views.business-settings.general_settings');
    }
     public function walletPointSettings(){
        return view('admin-views.business-settings.wallet_point_settings');
    }
     public function orderFeedbackEmailSettings(){
        return view('admin-views.business-settings.order_feedback_settings');
    }
    ...
    public function app_setting_update(Request $request)
    {
        ...
        if($request->platform == 'branch_settings')
        {
            
            DB::table('business_settings')->updateOrInsert(['key' => 'branch_menu_settings'], [
                'value' => json_encode([
                    'status' => $request['branch_menu']
        ...
        if($request->platform == 'develivery_employee_chef_settings')
        {
            
            DB::table('business_settings')->updateOrInsert(['key' => 'develivery_employee_chef_settings'], [
                'value' => json_encode([
                    'status' => $request['status']
        ...
        if($request->platform == 'branch_call_notification_settings')
        {
            
            DB::table('business_settings')->updateOrInsert(['key' => 'branch_call_notification_settings'], [
                'value' => json_encode([
                    'status' => $request['status']
        ...
        if($request->platform == 'customize_order_menu_update')
        {
            
            DB::table('business_settings')->updateOrInsert(['key' => 'customize_order_menu'], [
                'value' => json_encode([
                    'status' => $request['status']
         ...
         if($request->platform == 'fax_settings')
        {
            
            DB::table('business_settings')->updateOrInsert(['key' => 'fax_settings'], [
                'value' => json_encode([
                    'status' => $request['status']
        ...
        if($request->platform == 'wallet_point_settings')
        {
            $request->validate([
            ...
            DB::table('business_settings')->updateOrInsert(['key' => 'wallet_point_settings'], [
                'value' => json_encode([
                    'status' => $request['status'],
        ...
        if($request->platform == 'order_feedback_settings')
        {
            DB::table('business_settings')->updateOrInsert(['key' => 'order_feedback_settings'], [
                'value' => json_encode([
                    'status' => $request['status'],
        ...
        return back();
                ...
                $orders = Order::whereDate('delivery_date', $request->selected_date)->with(['customer'])->get();
                foreach($orders as $order){
                    Mail::to($order->customer->email)->send(new OrderFeedbackMail($order));
                }
            }
    ...
    public function update_delivery_fee(Request $request)
    {
        ...
        if($request['minimum_miles'] == null) {
            $request['minimum_miles'] = Helpers::get_business_settings('delivery_management')['minimum_miles'];
        }
        ...
        DB::table('business_settings')->updateOrInsert(['key' => 'delivery_management'], [
            'value' => json_encode([
                ...
                'minimum_miles' => isset($request['minimum_miles'])?$request['minimum_miles']:'',
            ]),
        ]);
