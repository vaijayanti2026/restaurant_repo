...
class OrderController extends Controller
{
        ...
        $order_count = [
            ...
            'failed' =>     Order::notPos()->notDineIn()->where(['order_status' => 'failed'])->notSchedule()
                ->when(!is_null($from) && !is_null($to), function ($query) use ($from, $to) {
                 ...
                 //       dd($query->orderBy('id', 'desc')->limit(10)->get());
    ...
    public function details($id)
    {
        ...
        $tipPrice=$order->tip_price!=''?$order->tip_price:0;
        ...
        // dd($reward['qty']);
        // }
        return view('admin-views.order.order-view', compact('order','tipPrice','coupnText', 'rewards'));
    }
    ...
    public function generate_invoice($id)
    {
        ...
        return view('admin-views.order.invoice', compact('order','coupnText', 'rewards'));
    }
    ...
    public function export_excel(Request $request)
    {
        ...
        foreach ($orders as $key => $order) {
            ...
            $data[] = array(
                ...
                'Delivery Charges' => Helpers::set_symbol($order['delivery_charge']),
                'Tip' => Helpers::set_symbol($order['tip_price']),
                'Payment Status' => $order->payment_status == 'paid' ? 'Paid' : 'Unpaid',
                'Tax'=>Helpers::set_symbol($order->total_tax_amount),
                ...
                'Payment Method'=>translate($order['payment_method']),
