...
class CustomerController extends Controller
{
    ...
    {
        try {
            $transitions = PointTransitions::latest()
                ->where(['user_id' => $request->user()->id])
                ->get();

            $orderIds = $transitions->map(function ($transition) {
                if (preg_match('/order ID\s*:\s*(\d+)/i', (string) $transition->description, $matches)) {
            ...
            $history = $transitions
                ->map(function ($transition) use (&$runningBalance, $orders) {
                    ...
                    $transition->balance_before = round($runningBalance + ($transition->is_credit ? -$amount : $amount), 2);
                    ...
                    $runningBalance += $transition->is_credit ? -$amount : $amount;
