...
class CustomerAuthController extends Controller
{
    ...
    public function registration(Request $request)
    {
        ...
        $user = User::create([
            ...
            'is_subscribed'=>isset($request->is_subscribed) && $request->is_subscribed=='1'?'1':'0',
            'date_of_birth'=>isset($request->date_of_birth) && $request->date_of_birth!=''?date('Y-m-d H:i:s',strtotime($request->date_of_birth)):null
        ]);
        
        if(isset($request->is_subscribed) && $request->is_subscribed=='1'){
        ...
        $email_verification = Helpers::get_business_settings('email_verification');
         Mail::to($user->email)->send(new \App\Mail\NewAccount($user));
    ...
    public function guest_login(Request $request)
    {
        ...
        $user = new User();
        $user->f_name = trim((string) $request->input('f_name', '')) ?: 'Guest';
        $user->l_name = trim((string) $request->input('l_name', '')) ?: 'Customer';
        $user->email = $email ?: 'guest-' . Str::uuid() . '@guest.local';
        ...
        $user->login_medium = 'guest';
        $user->save();

        $token = $user->createToken('RestaurantGuestCustomerAuth')->accessToken;
