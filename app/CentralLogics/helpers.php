...
class Helpers
{
    ...
    /**
     * Return the current time in the restaurant's configured timezone.
     *
     * Order timestamps are persisted as DATETIME values, so using the
     * configured business timezone here keeps every order entry consistent
     * even when PHP or the database server runs in another timezone.
     */
    public static function order_now()
    {
        $timezone = self::get_business_settings('time_zone') ?: config('app.timezone', 'America/Chicago');

        try {
            return Carbon::now($timezone);
        } catch (\Exception $exception) {
            return Carbon::now(config('app.timezone', 'America/Chicago'));
        }
    }
    ...
    {
        if (empty($fcm_token)) {
            Log::warning('Push notification skipped because the device token is empty.');
            return false;
        }

        return self::send_firebase_message('token', (string) $fcm_token, $data, data_get($data, 'type', 'general'));
    }

    public static function send_push_notif_to_topic($data, $topic, $type)
    {
        $topic = preg_replace('#^/topics/#', '', (string) $topic);

        if (empty($topic)) {
            Log::warning('Push notification skipped because the Firebase topic is empty.');
            return false;
        }

        return self::send_firebase_message('topic', $topic, $data, $type);
    }

    private static function send_firebase_message($target_type, $target, $data, $type)
    {
        $log_target = $target_type === 'token'
            ? substr((string) $target, 0, 8) . '...'
            : $target;
        $title = (string) data_get($data, 'title', '');
        $description = (string) data_get($data, 'description', data_get($data, 'body', ''));
        $image = (string) data_get($data, 'image', '');
        $order_id = data_get($data, 'order_id');

        if ($title === '' && $description === '') {
            Log::warning('Push notification skipped because both title and description are empty.', [
        ...
        if ($order_id !== null && $order_id !== '') {
            $message_data['order_id'] = (string) $order_id;
        }
